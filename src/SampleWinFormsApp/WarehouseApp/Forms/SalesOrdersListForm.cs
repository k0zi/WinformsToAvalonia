...
public partial class SalesOrdersListForm : ListFormBase<SalesOrder>
{
    public SalesOrdersListForm()
    {
        InitializeComponent();
        statusFilterComboBox.Items.Add("All Statuses");
        foreach (var status in Enum.GetNames<SalesOrderStatus>())
        {
    ...
    protected override async Task<List<SalesOrder>> LoadDataAsync(string? searchText)
    {
    ...
    protected override void EditEntity(SalesOrder entity)
    {
    ...
    protected override async Task DeleteEntityAsync(SalesOrder entity)
    {
        using var ctx = Db.CreateContext();
