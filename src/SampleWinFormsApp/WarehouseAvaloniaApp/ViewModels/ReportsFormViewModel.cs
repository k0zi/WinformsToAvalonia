...
/// <summary>
/// ViewModel for ReportsForm (user customizations).
/// This file is preserved during reconversion - add your custom code here.
/// </summary>
public partial class ReportsFormViewModel : CommunityToolkit.Mvvm.ComponentModel.ObservableObject
{
    internal List<(string Label, double Value)> _lastChartData = [];
    ...
    internal async Task LoadAuditLogAsync()
        {
    ...
    internal async void exportCsvButton_Click(object? sender, EventArgs e)
        {
            if (saveFileDialog.ShowDialog(this) != WarehouseAvaloniaApp.Common.DialogResult.OK)
            {
                return;
            }
            ...
            await WarehouseAvaloniaApp.Common.Dialogs.ShowAsync($"Exported to {saveFileDialog.FileName}","Export Complete",WarehouseAvaloniaApp.Common.MessageBoxButtons.OK,WarehouseAvaloniaApp.Common.MessageBoxIcon.Information);
        }

    internal void chooseFontButton_Click(object? sender, EventArgs e)
        {
            fontDialog.Font = salesChart.TitleFont;
            if (fontDialog.ShowDialog(this) == WarehouseAvaloniaApp.Common.DialogResult.OK)
            {
    ...
    internal void printPreviewButton_Click(object? sender, EventArgs e)
        {
