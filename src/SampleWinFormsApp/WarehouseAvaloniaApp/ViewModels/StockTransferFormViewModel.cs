    ...
    internal async Task PostTransferAsync()
        {
            ...
            {
                await WarehouseAvaloniaApp.Common.Dialogs.ShowAsync($"Could not post transfer: {ex.Message}","Post Failed",WarehouseAvaloniaApp.Common.MessageBoxButtons.OK,WarehouseAvaloniaApp.Common.MessageBoxIcon.Error);
            }
            finally
            {
