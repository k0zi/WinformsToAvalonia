    ...
    internal async Task PostReceiptAsync()
        {
            ...
            {
                await ConvertedAvalonia.Common.Dialogs.ShowAsync($"Could not post receipt: {ex.Message}","Post Failed",ConvertedAvalonia.Common.MessageBoxButtons.OK,ConvertedAvalonia.Common.MessageBoxIcon.Error);
            }
            finally
            {
